export function actualizarTextoIndicePokemones(texto: string) {
  const $indice = <HTMLDivElement>document.querySelector('#indice');
  $indice.textContent = texto;
}

export function mostrarListadoPokemones(nombresPokemones: string[], pokemonSeleccionadoCallback = (nombre: string) => {}) {
  const $indice = <HTMLDivElement>document.querySelector('#indice');
  $indice.innerHTML = '';
